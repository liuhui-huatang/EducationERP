package com.htcompany.educationerpforgansu.commonpart.views.calendarview;
...
public class MonthDateView extends View {
	...
	private int mSelectBGColor = Color.parseColor("#26b8f2");
	private int mCurrYear,mCurrMonth,mCurrDay;
	private int mSelYear,mSelMonth,mSelDay;
	private int mColumnSize,mRowSize;
	private DisplayMetrics mDisplayMetrics;
	private int mDaySize = 14;//日期字体大小
	private TextView tv_date,tv_week;
	private int weekRow;
	private int [][] daysString;
	private int mCircleRadius = 3;//选中圆的大小
	private DateClick dateClick;
	private int mCircleColor = Color.parseColor("#26b8f2");
...
//	wujiaxing			canvas.drawRect(startRecX, startRecY, endRecX, endRecY, mPaint);
				canvas.drawCircle((startRecX+endRecX)/2, (startRecY+endRecY)/2, (endRecX-startRecX)/3-5, mPaint);
				//记录第几行，即第几周
				weekRow = row + 1;
			}
			//绘制事务圆形标志
...
//				mPaint.setColor(mCurrentColor);
				mPaint.setColor(getResources().getColor(R.color.work_bottomtv2_color));
			}else{
			...
			if(tv_date != null){
				if((mSelMonth + 1)>9){
					...
					tv_date.setText(mSelYear + "-0" + (mSelMonth + 1));
				}
			}
